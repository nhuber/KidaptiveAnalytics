/*

Averages prompt correctness across number of prompts learners have seen in Gyroracer for learners who have
seen at least 10 prompts.

Note: Needs to be run separately for green light prompts (prompt_id = 6122) and red lights (prompt_id = 6121)

*/

SELECT prompt_count, AVG(CAST(correct AS INT)), COUNT(*) FROM

(SELECT *, ROW_NUMBER() OVER(PARTITION BY learner_id ORDER BY created) AS prompt_count FROM
        (SELECT *,
        CASE
                WHEN SUBSTRING(json, STRPOS(json, '\"1058\"') + CHAR_LENGTH('\"1058\":['), 4) = '2394' THEN '1'
                ELSE '0'
        END AS correct
        FROM
                (SELECT * FROM
                        (SELECT learner_id, created, json,
                        SUBSTRING(json, STRPOS(json, '"promptId":') + CHAR_LENGTH('"promptId":'), 4) AS prompt_id
                        FROM result) AS all_prompts
                WHERE prompt_id IN ('6121')) AS red_gyroracer) AS w_correct) AS w_prompt_count

INNER JOIN

                (SELECT learner_id FROM
                (SELECT learner_id, COUNT(*) AS num_prompts FROM
                (SELECT * FROM
                        (SELECT learner_id, created, json,
                        SUBSTRING(json, STRPOS(json, '"promptId":') + CHAR_LENGTH('"promptId":'), 4) AS prompt_id
                        FROM result) AS all_prompts2 
                WHERE prompt_id IN ('6121')) AS red_gyroracer2
                GROUP BY learner_id) AS good_learners
                WHERE num_prompts >= 10) AS good_learners2

ON (w_prompt_count.learner_id = good_learners2.learner_id)
                
GROUP BY prompt_count;
